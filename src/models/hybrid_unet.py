...
class HybridUNet(nn.Module):
    """
    Hybrid U-Net combining 3D density with sequence features for RNA motif classification.
    ...
    Args:
        n_classes: Number of output classes (default: 15)
    ...
    def forward(self, density, sequence):
        ...
        fused = torch.cat([density_features, sequence_features], dim=1)  # (B, 512)
    ...
    def get_feature_representations(self, density, sequence):
        """
        ...
        fused = torch.cat([density_features, sequence_features], dim=1)
...
def test_model():
    ...
    with torch.no_grad():
        features = model.get_feature_representations(density, sequence)
    ...
    print(f"\nFusion & Classifier:")
    print(f"  Concatenate: (B, 256) + (B, 256) → (B, 512)")
    print(f"  MLP: 512 → 256 → 128 → 15")
